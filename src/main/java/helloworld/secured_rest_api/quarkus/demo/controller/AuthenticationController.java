...
public class AuthenticationController {
    ...
    public Uni<TokenDTO> authenticate(AuthenticationRequestDTO authenticationRequest) {
        ...
        return Uni.createFrom().item(authenticationRequest)
                // Authenticate user
                .onItem()
                .transform(credentialsDTO -> _authenticationService.authenticate(credentialsDTO))
                // Generate token for user
                .onItem()
                .transform(securityIdentity -> _tokenService.generateToken(securityIdentity))
                // Map to DTO for caller
                .onItem()
                .transform(token -> new TokenDTO(token, Constants.KEY_BEARER, _jwtExpiration));
    }
}
